...
namespace KEngine.UI
{
    ...
    public class UIModule
    {
        ...
        public UILoadState OpenWindow(Type type,string uiTemplateName, string instanceName, UIController parent, params object[] args)
        {
            string uiName = uiTemplateName; // 去掉"CUI"
            UILoadState uiState;
            if (!UIWindows.TryGetValue(uiTemplateName, out uiState))
            {
                uiState = LoadWindow(type,uiTemplateName, true, parent, args);
                return uiState;
            }
            uiState.UIType = type;

            if (uiState.UIWindow != null && uiState.UIWindow.IsCloseDestory)//如果关闭销毁
            {
                return OpenDynamicWindow(type,uiTemplateName, instanceName, parent, args);
            }else
            {
                uiState.ParentWindow = parent;
                OnOpen(uiState, args);
                return uiState;
            }
        }
        ...
        public UILoadState OpenWindow<T>(string Path,string instanceName, UIController parent, Action<UIController, object[]> loadCallBack,params object[] args) where T : UIController
        {
            UILoadState state = OpenWindow(typeof(T), Path, instanceName, parent, args);
            if (loadCallBack != null)
            {
                state.DoCallback(loadCallBack);
            }
            return state;
        }
        ...
        // 隐藏时打开，打开时隐藏
        public void ToggleWindow<T>(string name,UIController Parent = null,params object[] args)
        {
            ToggleWindow(typeof(T),name, Parent, args);
        }

        private void ToggleWindow(Type type,string name,UIController Parent = null, params object[] args)
        {
            if (IsOpen(name))
            {
                CloseWindow(name);
            ...
            {
                OpenWindow(type, name,name,Parent, args);
            }
        }
        ...
        /// // Dynamic动态窗口，复制基准面板
        /// </summary>
        private UILoadState OpenDynamicWindow(Type type,string template, string instanceName,UIController Parent = null, params object[] args)
        {
            ...
            if (!UIWindows.TryGetValue(instanceName, out uiInstanceState)) // 实例创建
            {
                ...
                uiInstanceState.ParentWindow = Parent;
                uiInstanceState.UIType = type;
                UIWindows[instanceName] = uiInstanceState;
            }

            CallUI<UIController>(template, (_ui, _args) =>
            {
                // _args useless
        ...
        public void CloseWindow(string name)
        {
            ...
            //{
            uiState.UIWindow.CachedGameObject.SetActive(false);
            uiState.ParentWindow = null;
            uiState.UIWindow.CachedTransform.SetParent(UIParent.transform);
        ...
        public UILoadState LoadWindow(Type type,string windowTemplateName, bool openWhenFinish,UIController parent, params object[] args)
        {
        ...
        private IEnumerator LoadUIAssetBundle(string name, UILoadState openState, KCallback callback = null)
        {
            ...
            var uiBase = UiBridge.CreateUIController(openState.UIType,uiObj, openState.TemplateName);
        ...
        private void DestroyWindow(string uiTemplateName)
        {
            UILoadState uiState;
        ...
        // 使用泛型方式
        public void CallUI<T>(string uiTemplateName,Action<UIController, object[]> callback,UIController Parent, params object[] args) where T : UIController
        {
            Debuger.Assert(callback);
            ...
            if (!UIWindows.TryGetValue(uiTemplateName, out uiState))
            {
                uiState = LoadWindow(typeof(T),uiTemplateName, false, Parent); // 加载，这样就有UIState了, 但注意因为没参数，不要随意执行OnOpen
            }
        ...
        private void OnOpen(UILoadState uiState, params object[] args)
        {
            ...
            if (uiState.ParentWindow != null)//如果有父窗口添加到父窗口下面
            {
                ...
                uiBase.CachedTransform.SetAsLastSibling();
            }
            else
            {
            ...
            //if (uiState.ParentWindow != null)//如果是主窗口，
            //{
            //    uiState.DoCallback((ui, agr) =>
            //    {
            //        if (CurMainUI != null)
            //        {
            //            CurMainUI.UIWindow.CachedGameObject.SetActive(false);
            //        }
            //    }, null);
            //}
            //uiState.DoCallback((ui, agr) =>
            ...
            //}, null);
            if (uiState.ParentWindow == null)//设置当前主窗口
            {
                CurMainUI = uiState;
            }
        ...
        public UILoadState CurMainUI;
        /// <summary>
        /// 初始化窗口
        /// </summary>
        /// <param name="uiState"></param>
        /// <param name="uiBase"></param>
        /// <param name="open">初始化后是否显示</param>
        /// <param name="args"></param>
        ...
        public void CloseCurMainWindow()
        {
            if(CurMainUI != null)
            {
                CloseWindow(CurMainUI.TemplateName);
                CurMainUI = null;
            }
        }

        public UILoadState GetCurMainState()
        {
            return CurMainUI;
        }
    }
