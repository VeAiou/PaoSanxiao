...
namespace KSFramework
{
    ...
    public class LuaUIController : UIController
    {
        ...
        public CSUIController mCsControl;
        public UILuaOutlet outlet;//组件
        ...
        bool CheckInitScript(bool showWarn = false)
        {
            ...
            if (!IsUseCs && mCsControl)//C#和lua只能使用一种
                DestroyImmediate(mCsControl);

            outlet = this.GetComponent<UILuaOutlet>();
            //暂时关闭slua的脚本读取逻辑
            return false;
            if (!IsCachedLuaTable)
            {
        ...
        {
            return GetControl(typeName, uri, findTrans,true);
        }
